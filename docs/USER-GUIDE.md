# KuLoan Ledger Administrator User Guide

## 1. Introduction

KuLoan Ledger is a WordPress dashboard plugin for managing private loans. It helps the administrator track who borrowed money, how much was lent, the monthly interest, the payment due date, partial payments, loan extensions, penalties, signed acknowledgement documents, and the complete transaction history of each loan.
...
## 2. Installing KuLoan Ledger

1. Log in to WordPress as an Administrator.
2. Go to **Plugins → Add New**.
3. Click **Upload Plugin**.
4. Select the KuLoan Ledger ZIP file.
5. Click **Install Now**.
6. After installation, click **Activate Plugin**.
7. A new **KuLoan Ledger** menu appears in the WordPress dashboard.
...
> **Important:** Lending income is not the same as accounting profit. KuLoan Ledger does not currently deduct operating expenses, bad-debt write-offs, taxes, staff costs, or other business expenses.
...
1. Go to **KuLoan Ledger → Borrowers**.
2. Find the borrower.
3. Click **Edit**.
...
Updated borrower data is used when KuLoan Ledger generates future loan documents. Any signed acknowledgement file that was already uploaded remains unchanged and continues to serve as the historical signed record.
...
Open any loan from **KuLoan Ledger → Loans**.

At the top of the loan screen, KuLoan Ledger shows four current figures:
...
KuLoan Ledger automatically allocates each payment in this order:
...
Go to **KuLoan Ledger → Loans** and click **Export CSV**.
...
A simple routine for using KuLoan Ledger is:
...
KuLoan Ledger may hold personally identifiable borrower information and financial records.
...
Confirm that at least one borrower has already been created under **KuLoan Ledger → Borrowers**.

### The interest rate is not 20%
Open **KuLoan Ledger → Settings** and confirm **Default Monthly Interest (%)** is set to `20`. Also remember that an individual loan can have a different rate if its value was changed during loan creation.
...
KuLoan Ledger retains the v1.1.0 dedicated follow-up workflow so you do not have to manually remember which borrowers need attention.
...
3. KuLoan Ledger sends a plain-text email through WordPress `wp_mail()`.
...
Go to **KuLoan Ledger → Settings → Due-Date Reminders**.
...
KuLoan Ledger uses WP-Cron only to trigger the reminder digest. Loan balances and due-date calculations do not depend on WP-Cron. Because normal WP-Cron runs when the WordPress site receives traffic, the digest should be treated as daily rather than guaranteed at an exact clock time.

### Recommended Daily Routine

1. Open **KuLoan Ledger → Dashboard**.
2. Review **Due Today**, **Due This Week**, **Grace Period**, and **Overdue** cards.
3. Open **KuLoan Ledger → Reminders**.
...
KuLoan Ledger provides administrative calculations and record keeping. It does not determine whether an interest rate, penalty, acknowledgement, or lending arrangement is legally enforceable.
