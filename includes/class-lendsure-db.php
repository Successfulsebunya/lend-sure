...
class LendSure_DB {
    ...
    private static function migrate( $previous_version ) {
        if ( $previous_version && version_compare( $previous_version, '1.2.0', '<' ) ) {
            ...
            // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching -- One-time migration write; activate() flushes the KuLoan Ledger object cache after migration.
            $wpdb->query(
                $wpdb->prepare(
    ...
    public static function update( $table_name, $data, $where, $format = null, $where_format = null ) {
        ...
        // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching -- Plugin-owned table write; the KuLoan Ledger object cache is flushed immediately after a successful update.
