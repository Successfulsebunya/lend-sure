...
class LendSure_Reminders {
    ...
    private function send_mail_safely( $to, $subject, $message ) {
        ...
        if ( ! $sent && '' === $this->mail_error ) {
            $this->mail_error = __( 'WordPress mail returned false. Check the site SMTP/mail configuration.', 'kuloan-ledger' );
        }
    ...
    public static function timing_status( $due_date ) {
        ...
        if ( ! $due ) {
            return array( 'key' => 'unknown', 'label' => __( 'Unknown', 'kuloan-ledger' ), 'days' => 0 );
        }
        ...
        if ( 0 === $days ) {
            return array( 'key' => 'due_today', 'label' => __( 'Due Today', 'kuloan-ledger' ), 'days' => 0 );
        }
        if ( $days > 0 && $days <= 7 ) {
            return array( 'key' => 'due_week', 'label' => __( 'Due This Week', 'kuloan-ledger' ), 'days' => $days );
        }
        if ( $days < 0 && abs( $days ) <= $grace ) {
            return array( 'key' => 'grace', 'label' => __( 'Grace Period', 'kuloan-ledger' ), 'days' => $days );
        }
        if ( $days < 0 ) {
            return array( 'key' => 'overdue', 'label' => __( 'Overdue', 'kuloan-ledger' ), 'days' => $days );
        }
        return array( 'key' => 'upcoming', 'label' => __( 'Upcoming', 'kuloan-ledger' ), 'days' => $days );
    }
    ...
    public function send_daily_digest() {
        ...
        foreach ( $rows as $loan ) {
            ...
            $items[] = sprintf(
                ...
                __( '#%1$d %2$s — %3$s %4$s — due %5$s — %6$s', 'kuloan-ledger' ),
                $loan->id,
                $loan->full_name,
    ...
    public function send_borrower_reminder() {
        ...
        if ( ! $loan || 'active' !== $loan->status || ! is_email( $loan->email ) ) {
            wp_die( esc_html__( 'A valid borrower email is required for this reminder.', 'kuloan-ledger' ) );
        }
        ...
        $subject = sprintf( __( 'Loan reminder — due %s', 'kuloan-ledger' ), $loan->due_date );
...
', 'kuloan-ledger' ),
        ...
        $borrower_log    = $message . ( $borrower_result['error'] ? "\n\n" . __( 'Mail error:', 'kuloan-ledger' ) . ' ' . $borrower_result['error'] : '' );
        ...
        if ( is_email( $lender_email ) && strtolower( $lender_email ) !== strtolower( $loan->email ) ) {
            ...
            $copy_subject = sprintf( __( 'Copy: loan reminder sent to %s', 'kuloan-ledger' ), $loan->full_name );
...
%3$s', 'kuloan-ledger' ),
            ...
            $copy_log    = $copy_message . ( $copy_result['error'] ? "\n\n" . __( 'Mail error:', 'kuloan-ledger' ) . ' ' . $copy_result['error'] : '' );
            $this->log( $loan_id, 'lender_copy_email', $lender_email, $copy_sent ? 'sent' : 'failed', $copy_log );
        }

        if ( $sent && $copy_sent ) {
            $notice = __( 'Reminder email sent to the borrower and a lender/admin copy was sent where configured.', 'kuloan-ledger' );
        } elseif ( $sent ) {
            $notice = __( 'Borrower reminder email sent, but the lender/admin copy could not be sent.', 'kuloan-ledger' );
        } else {
            $notice = __( 'WordPress could not send the borrower reminder email. Check the site mail/SMTP configuration.', 'kuloan-ledger' );
